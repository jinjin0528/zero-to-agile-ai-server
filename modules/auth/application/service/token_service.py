...
class TokenService:
    ...
    def _create_token(self, abang_user_id: int, secret: str, expires_in_seconds: int) -> str:
    ...
    def create_tokens(self, abang_user_id: int) -> tuple[str, str]:
        """
        로그인 직후 호출:
          - Access / Refresh 생성
          - Refresh는 Redis에 저장
        """
        access = self._create_token(abang_user_id, self.access_secret, self.ACCESS_TTL_SECONDS)
        refresh = self._create_token(abang_user_id, self.refresh_secret, self.REFRESH_TTL_SECONDS)

        # 기존 refresh 덮어쓰기
        self.token_repo.save_refresh(abang_user_id, refresh, ttl_seconds=self.REFRESH_TTL_SECONDS)
    ...
    def rotate_refresh(self, refresh_token: str) -> tuple[int, str, str]:
        ...
        try:
            ...
            abang_user_id = int(payload.get("abang_user_id"))
        except jwt.ExpiredSignatureError:
            raise HTTPException(status_code=401, detail="REFRESH_TOKEN_EXPIRED")
        except jwt.PyJWTError:
            raise HTTPException(status_code=401, detail="REFRESH_TOKEN_INVALID")

        stored = self.token_repo.get_refresh(abang_user_id)
        if stored is None:
            raise HTTPException(status_code=401, detail="REFRESH_TOKEN_NOT_FOUND")

        if stored != refresh_token:
            self.token_repo.delete_refresh(abang_user_id)
            raise HTTPException(status_code=401, detail="REFRESH_TOKEN_REUSED")

        new_access, new_refresh = self.create_tokens(abang_user_id)
        return abang_user_id, new_access, new_refresh

    def logout(self, abang_user_id: int):
        self.token_repo.delete_refresh(abang_user_id)
    ...
    def verify_access_token(self, access_token: str) -> int:
        ...
        try:
            ...
            return int(payload.get("abang_user_id"))
        except jwt.ExpiredSignatureError:
            raise HTTPException(status_code=401, detail="TOKEN_EXPIRED")
        except jwt.PyJWTError:
            raise HTTPException(status_code=401, detail="TOKEN_INVALID")

    def get_abang_user_id_from_refresh(self, refresh_token: str) -> int | None:
        try:
            payload = jwt.decode(refresh_token, self.refresh_secret, algorithms=["HS256"])
            return int(payload.get("abang_user_id"))
        except jwt.PyJWTError:
