...
from modules.house_platform.application.dto.house_platform_dto import HousePlatformUpsertModel, HousePlatformOptionUpsertModel
...
class ObservationRawAssembler:
    ...
    def build_price_raw(
        ...
        recommendation_observation_id: int | None,
    ) -> dict:
        total_monthly_cost = (house.monthly_rent or 0) + (house.manage_cost or 0)
        return {
            "id": None,
            ...
            "nonlinear_price_burden": None,
        }

    @staticmethod
    def build_convenience_raw(options: HousePlatformOptionUpsertModel) -> dict:
        coverage = sum([
        ...
        return {"essential_option_coverage": coverage, "convenience_score": coverage}

    @staticmethod
    def empty_convenience_raw() -> dict:
        return {"essential_option_coverage": 0.0, "convenience_score": 0.0}

    @staticmethod
    def build_distance_summary_raw(distances: Iterable[DistanceFeatureObservation]) -> dict:
        if not distances:
            return {
    ...
    @staticmethod
    def build_risk_raw(house: HousePlatformUpsertModel) -> dict:
        return {
            "is_banned": bool(house.is_banned),
            "floor_ratio": (house.floor_no / house.all_floors if house.floor_no and house.all_floors else None),
